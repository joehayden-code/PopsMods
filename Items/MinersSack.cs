        ...
        private void RollOreDrops(Player player)
        {
            ...
            if (isHardmode)
            {
                ...
                else if (roll < 45)
                {
                    ...
                    int oreType = Main.rand.NextBool() ? ItemID.CobaltOre : ItemID.PalladiumOre;
                    player.QuickSpawnItem(player.GetSource_ItemUse(Item), oreType, amount);
                }
                else if (roll < 75)
                {
                    int amount = Main.rand.Next(5, 12);
                    int oreType = Main.rand.NextBool() ? ItemID.MythrilOre : ItemID.OrichalcumOre;
                    player.QuickSpawnItem(player.GetSource_ItemUse(Item), oreType, amount);
                }
                else
                {
                    int amount = Main.rand.Next(5, 12);
                    int oreType = Main.rand.NextBool() ? ItemID.AdamantiteOre : ItemID.TitaniumOre;
                    player.QuickSpawnItem(player.GetSource_ItemUse(Item), oreType, amount);
                }
            }
            else
            {
                ...
                {
                    int amount = Main.rand.Next(5, 12);
                    int oreType = Main.rand.NextBool() ? ItemID.CopperOre : ItemID.TinOre;
                    player.QuickSpawnItem(player.GetSource_ItemUse(Item), oreType, amount);
                }
                else if (roll < 75)
                {
                    int amount = Main.rand.Next(5, 12);
                    int oreType = Main.rand.NextBool() ? ItemID.IronOre : ItemID.LeadOre;
                    player.QuickSpawnItem(player.GetSource_ItemUse(Item), oreType, amount);
                }
                else
                {
                    int amount = Main.rand.Next(5, 12);
                    int oreType = Main.rand.NextBool() ? ItemID.GoldOre : ItemID.PlatinumOre;
                    player.QuickSpawnItem(player.GetSource_ItemUse(Item), oreType, amount);
                }
            }
        }
